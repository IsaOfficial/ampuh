    ...
    function adminExcelStatusText(array $item): string
    {
        ...
        if ($status === 'approved') {
            ...
            if (!empty($item['verification_token'])) {
                $text .= ' - Kode: ' . substr($item['verification_token'], 0, 12);
            }
            return $text;
        }
...
<style>
    ...
    .header-title { font-size: 16px; text-align: center; font-weight: bold; }
    .info { font-size: 13px; line-height: 1.6; }
    td img { display: block; margin: 0 auto 4px; max-width: 80px; max-height: 80px; }
</style>

<table>
    <tr>
        <th colspan="8" class="header-title">LAPORAN KEGIATAN PEGAWAI</th>
    </tr>
    <tr>
        <th colspan="8" class="header-title">Rekapitulasi Laporan Kegiatan</th>
    </tr>
    <tr>
    ...
    <?php if (empty($laporan)): ?>
        <tr>
            <td colspan="8" class="text-center">Tidak ada data laporan pada rentang tanggal ini.</td>
        </tr>
    <?php else: ?>
        <?php $no = 1; ?>
        <?php foreach ($laporan as $item): ?>
            <tr>
                ...
                <td class="text-center">
                    <?php if (!empty($item['foto_pegawai']) && preg_match('/\.(jpg|jpeg|png)$/i', $item['foto_pegawai'])): ?>
                        <img src="<?= 'http://localhost/ampuh.mtsn1jepara.sch.id/public/uploads/foto/' . $item['foto_pegawai'] ?>" width="50" height="50" alt="<?= htmlspecialchars($item['foto_pegawai']) ?>"><br>
                    <?php endif; ?>
                    <?= htmlspecialchars($item['foto_pegawai'] ?? '-') ?>
                </td>
                <td class="text-start"><?= htmlspecialchars($item['nama_pegawai'] ?? '-') ?></td>
                ...
                <td class="text-center">
                    <?php if (!empty($item['bukti'])): ?>
                        <?php if (preg_match('/\.(jpg|jpeg|png)$/i', $item['bukti'])): ?>
                            <img src="<?= 'http://localhost/ampuh.mtsn1jepara.sch.id/public/uploads/bukti/' . $item['bukti'] ?>" width="100" alt="<?= htmlspecialchars($item['bukti']) ?>"><br>
                        <?php endif; ?>
                ...
                <td class="text-center"><?= htmlspecialchars(adminExcelStatusText($item)) ?></td>
            </tr>
        <?php endforeach; ?>
    <?php endif; ?>
</table>